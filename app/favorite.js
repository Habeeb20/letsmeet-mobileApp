...
import { getFavorites } from '../constants/api';
...
const Favorite = () => {
  ...
  useEffect(() => {
    const fetchFavorites = async () => {
      ...
      try {
        ...
        const response = await getFavorites(storedToken);
        setFavoriteUsers(response.data);
      } catch (err) {
        setError(err.response?.data?.message || 'Failed to fetch favorite users');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  <Text style={styles.subtitle}>{favoriteUsers.length ? `${favoriteUsers.length} saved profiles` : 'None saved yet'}</Text>
</View>
<SubNav active="favorites" />
      {favoriteUsers.length > 0 ? (
        ...
        <View style={styles.centerScreen}>
          <Ionicons name="star-outline" size={48} color="#FF3D77" />
          <Text style={styles.noUsersText}>No favorite users yet</Text>
          <Text style={styles.noUsersSubText}>Add favorites from your matches!</Text>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  chip: { backgroundColor: '#E1F0FF', paddingHorizontal: 13, paddingVertical: 8, borderRadius: 12 },
